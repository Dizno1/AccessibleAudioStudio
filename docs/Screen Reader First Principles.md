...
- **Never move keyboard focus unexpectedly.** Focus only moves as the direct, predictable result of something the user did (e.g. completing a save moves focus to the confirmation/next step). It never jumps because of a background timer, an animation, or an unrelated event. This includes indirect causes: disabling the control that currently has focus, or destroying and recreating it, forces focus away just as surely as an explicit `.focus()` call elsewhere — and is worse, because it's an accident rather than a deliberate handoff. **Once recording or playback is underway, the control that had focus keeps it until the user moves it themselves or takes an action that deliberately hands focus somewhere else** (e.g. Stop moving focus to the Play button — an established, intentional exception, not a violation). Toggling a button's label (Start ↔ Stop) or a checkbox-like control's pressed state must update the existing element in place; it must never be disabled, removed, or recreated as a side effect of the state it's reflecting.
...
- **Don't disable a control just because changing it wouldn't currently do anything.** If disabling it risks moving focus away from wherever the user happens to be (a very live risk for the microphone/profile selectors right before or during a recording), leaving it enabled and simply ignoring the interaction is the safer, quieter choice.
