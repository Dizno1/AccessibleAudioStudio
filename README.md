...
- **Push a Pro version tag** (e.g. `git tag pro-v0.2.1 && git push origin pro-v0.2.1`) to build both installers and open a **draft** GitHub Release with them attached, ready to review and publish.
...
| Version | 1.0.0 | 0.2.1 (current test build — see "Pro version numbering" below) |
...
This took four tries to get right, and each one was grounded in what the
...
- **0.1.5** stopped using `tauri-plugin-dialog` for this function
  altogether, calling Windows' modern `IFileOpenDialog` COM API with
  `FOS_ALLOWMULTISELECT` directly via the small, Windows-only `wfd`
  crate. Real Windows testing reported exactly the same symptom a third
  time — proof this wasn't specific to `tauri-plugin-dialog`, since `wfd`
  is an independent implementation of a different (if related) API.
- **0.1.6** switched to a genuinely different *class* of Windows API:
  `GetOpenFileNameW` with `OFN_EXPLORER | OFN_ALLOWMULTISELECT` (see
  `pick_files_native` in `src-tauri/src/main.rs`) — the classic, non-COM
  Common Dialog Box Library mechanism, rather than a third variation on
  `IFileOpenDialog`. This is the same class of API Audacity's own Windows
  build uses for this exact dialog. Selected files come back from Windows
  as one buffer of NULL-separated strings terminated by a double-NULL —
  parsed by `parse_multi_select_buffer`, which was unit-tested in
  isolation (real `rustc`, not simulated) against the single-file case,
  the multi-file case, cancellation, filenames containing spaces, and the
  exact 15-file selection from real testing, before being included here.
  `wfd` is no longer a dependency; the `windows` crate
  (`Win32_UI_Controls_Dialogs` feature) is used instead, scoped to
  `[target.'cfg(windows)'.dependencies]` in `src-tauri/Cargo.toml` since
  this app only ships as Windows installers. `app.withGlobalTauri` remains
  `true` in `tauri.conf.json`, since the frontend still calls this one
  command via `window.__TAURI__.core.invoke(...)`.
- **0.1.7** discovered the real testing workflow was different from what
  every prior build had addressed: not selecting multiple files inside
  the dialog's own list, but copying files in File Explorer (Ctrl+C),
  opening the dialog, and pasting into its File Name field (Ctrl+V).
  That specific gesture cannot work by pasting text, as a matter of how
  Windows itself works — Explorer's multi-file copy places `HDROP` (a
  binary format) on the clipboard, and never a text format at all, per
  Microsoft's own account of this exact question (Raymond Chen,
  "Windows Explorer Doesn't Do Text"). Any plain text field's paste
  handling (`WM_PASTE`) explicitly requires `CF_UNICODETEXT`/`CF_TEXT` to
  insert anything, so no ordinary edit control on Windows — this app's
  dialog included — can recover a multi-file list this way. Fixed by
  checking the clipboard for a copied Explorer file list (`CF_HDROP`,
  read via `OpenClipboard`/`GetClipboardData`/`DragQueryFileW`) the
  moment Open Audio is activated; if two or more files are found there,
  they're used directly and the dialog is never shown for that
  activation. See `read_clipboard_file_list` in `src-tauri/src/main.rs`
  for the full explanation, including what was deliberately *not*
...
- `0.2.1` — the first real Windows test of 0.2.0 passed cleanly (installer worked, separate editor windows opened with correct titles, Alt+Tab worked, direct multi-select inside the dialog worked) and isolated the one remaining defect precisely: selecting multiple files *directly inside* the native dialog's own list already worked and correctly populated the File Name field with a quoted multi-name string, proving `GetOpenFileNameW`'s multi-name parsing, Rust's multi-path handling, and editor-window creation were all already correct — but pasting a copied Explorer file list into that same field still didn't (`WM_PASTE received by subclass: no`, per 0.1.8's diagnostics). Root cause: every previous build subclassed the outer ComboBoxEx32 control, but WM_PASTE is handled by its inner editable child, not the combo box itself — confirmed via Microsoft's own `GetComboBoxInfo`/`COMBOBOXINFO` documentation, whose `hwndItem` field is exactly "a handle to the edit box." Fixed by locating that inner child via `GetComboBoxInfo` (declared via raw FFI against `user32.dll`, not imported from `windows::Win32::UI::Controls` — the same module that failed to export `SetWindowSubclass` in 0.1.8 despite equally solid documentation) and subclassing it instead, with a fallback to the outer combo if the inner child can't be found. See `docs/Pro Roadmap.md`.
- `0.2.2`, … — subsequent capability builds (Save/Save-As dirty-state polish, cross-window duplicate-file detection, unsaved-changes-on-close protection are the next planned increments)
...
6. The published release's asset URLs (e.g. `.../releases/download/pro-v0.2.1/AccessibleAudioStudio Pro_0.2.1_x64_en-US.msi`) are stable direct-download links suitable for linking from OpenDoorDesign.org.

## Recommended next phase

Build 0.2.1 via GitHub Actions and reproduce the exact workflow the previous test found working for direct selection but not for paste: select several files in File Explorer, Ctrl+C, switch to AccessibleAudioStudio Pro, Ctrl+O, move to the File Name field, Ctrl+V. Open the Open Audio Diagnostics panel: "Inner editable child located" is the key new line — if it says yes and "WM_PASTE received by subclass" now also says yes (where 0.1.8 reported no), the fix is confirmed reaching the right control. Then check "Quoted filenames written to File name edit" and the recorded "Text written" against what the dialog itself already proved it can parse correctly during direct multi-select — if that same quoted text now gets typed into the field via paste, activating Open should produce the same multi-window result direct selection already does. If "Inner editable child located" says no, `GetComboBoxInfo` didn't find an edit child on this Windows build, and the code falls back to subclassing the outer combo as before — worth knowing either way. This environment has no Rust toolchain that can target Windows, so `GetComboBoxInfo`'s exact behavior against the real File Name control has not been verified here, only against Microsoft's own documentation. Also reconfirm the 0.2.0 acceptance test still passes in full (direct multi-select, editor window creation, Alt+Tab, window titles, cross-window clipboard) — nothing in that architecture was touched this round.
